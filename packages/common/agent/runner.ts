...
/**
 * 이 저장소의 패키지가 빌드해 둔 MCP 서버 번들을 stdio로 띄우는 설정을 만듭니다.
 *
 ...
 * @param entryPath - 빌드된 서버 진입점 (보통 `<package>/dist/server.js`)
 * @throws 번들이 없으면 빌드가 필요하다는 에러를 던집니다
 *
 * @example
 * ```ts
 * const servers = nodeMcpServer("mono-rele2-exchange", resolve(pkgRoot, "dist/server.js"));
 ...
 * 모두 `packages/common/.log/<taskId>.log`에 남습니다.
 *

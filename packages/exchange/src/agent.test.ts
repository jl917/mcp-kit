...
//
//   pnpm --filter @julong/mono-rele2-exchange build   # 에이전트가 띄울 MCP 서버
//   npx playwright install chromium                   # 최초 1회
//   pnpm test:agent
//
// 이 패키지는 MCP 도구만 제공합니다. 에이전트를 굴리는 쪽(모델·로깅·MCP 기동)은
// 전부 `@common/agent`에 있고, 여기서는 어떤 서버를 어떤 프롬프트로 부를지만 정합니다.
// ---------------------------------------------------------------------------

const PKG_ROOT = resolve(dirname(fileURLToPath(import.meta.url)), '..');
const REPO_ROOT = resolve(PKG_ROOT, '../..');
const SERVER_ENTRY = resolve(PKG_ROOT, 'dist/server.js');
...
/** 이 패키지가 빌드해 둔 MCP 서버를 띄워 에이전트를 한 번 실행합니다. */
function runAgent(taskId: string, prompt: string) {
  return runMcpAgent({
    taskId,
    prompt,
    systemPrompt: SYSTEM_PROMPT,
    servers: nodeMcpServer('mono-rele2-exchange', SERVER_ENTRY),
  });
}

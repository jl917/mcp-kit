...
export function getUser(user: string | RandomUser): string {
  const parsed: RandomUser = typeof user === 'string' ? JSON.parse(user) : user;
  ...
  getUserTool: toolDef({
    ...
    inputSchema: {
      user: z
        ...
        .describe(
          'RandomUser 형식의 JSON 문자열 또는 객체 — name.first / name.last / location.city 필수',
        ),
    },
    typeLabels: {
      user: 'RandomUser',
    ...
    returnType: 'string',
    returnDescription:
      '"이름은 {first} {last} 이고 현재 {city} 에 살고 있습니다." 형식의 한글 문장',
    handler: async ({ user }) => {
      try {
        const input: string | RandomUser = typeof user === 'string' ? user : (user as RandomUser);

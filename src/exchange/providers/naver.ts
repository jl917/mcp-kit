 ...
 */
export const scrapeNaver: Scraper = async (page, currencies, timeoutMs, deadline) => {
  ...
  for (const currency of currencies) {
    ...
    try {
      ...
      await card.waitFor({ state: 'attached', timeout: step });
      ...
      const raw = await readRateText(
        card.locator('[data-price-info] .price').first(),

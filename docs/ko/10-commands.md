...
두 워크플로의 `Build` 단계도 같은 시크릿을 받습니다. 빌드에 넘긴 자격 증명은 번들에 그대로 적히기
때문입니다. `release.yml`에서는 그게 의도한 동작입니다 — npm에 올라가는 패키지가 자격 증명을 들고
있으므로 `npx @julong/mcp-kit`는 클라이언트 설정에 아무것도 넣지 않아도 됩니다. 대신 설치한 사람은
누구나 그 키를 되찾을 수 있습니다. 어떤 키를 시크릿에 넣어야 하는지는
[빌드 시점 TMDB 자격 증명](02-tech-stack.md#빌드-시점-tmdb-자격-증명)을 보십시오.

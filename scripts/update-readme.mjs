...
import { generateReadmeApiDocs } from '../src/common/kit/skill.ts';
...
const branch = 'main';
const skillInstallCmd = `npx skills add ${repoBase}/tree/${branch}/skills`;

const toolsUrl = pathToFileURL(join(process.cwd(), 'src/tools/index.ts')).href;
const { tools, UTILS_ENV_KEYS } = await import(toolsUrl);

const cliBinName =
  Object.entries(pkg.bin ?? {}).find(([, v]) => String(v).endsWith('cli.js'))?.[0] ?? binCli;
